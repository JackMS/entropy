...
int main(int argc, char *argv[])
{
	...
	for (h=1; h < argc; ++h)
	{
		...
		fprintf(ofp, "ID, X_Pos, Y_Pos, Z_Pos, X_Vel, Y_Vel, Z_Vel, Temp, U, U_dot, rho, drho_dt \n");
		for (i = 0; i < nobj; ++i)
		{
			fread(&part,sizeof(particle),1,fp);
			fprintf(ofp,"%d, %g, %g, %g, %g, %g, %g, %g, %g, %g, %g, %g \n", 
				part.ident, part.x, part.y, part.z, part.vx, part.vy, part.vz, part.temp, part.u, part.udot, part.rho, part.drho_dt);
		}
